...
/**
 * Controller for handling user-related operations, primarily for syncing user data from an external
 * authentication provider like Clerk.
 */
@RestController
@RequestMapping("/api/v1/users") // Standard API versioning in the path
@Tag(
    name = "User Management",
    description = "Endpoints for syncing and retrieving user information")
...
public class UserController {
  ...
  /**
   * This is the core endpoint for integrating with Clerk. The frontend should call this endpoint
   * after a user successfully signs up or logs in. The service layer handles both creation and
  ...
  @Operation(
      ...
      description =
          "Creates a new user or updates an existing one based on the provided Clerk ID. This is the primary endpoint for user integration.")
  ...
  public ResponseEntity<UserDto> syncUser(
      Authentication authentication, @Valid @RequestBody UserSyncDto userSyncDto) {
  ...
  /**
   * Retrieves the profile of the currently authenticated user. The user's identity is determined
   * from the JWT token provided in the request.
   *
   * <p>If the user doesn't exist in the database yet (first-time access), this endpoint will
   * attempt to sync the user from JWT claims automatically.
   */
  @GetMapping("/me")
  @Operation(
      summary = "Get Current User Profile",
      description =
          "Retrieves the profile of the currently authenticated user. Auto-syncs from JWT if user doesn't exist.")
  @SecurityRequirement(name = "bearerAuth") // Indicates this endpoint requires authentication
  @ApiResponse(responseCode = "200", description = "Profile retrieved successfully")
  @ApiResponse(responseCode = "404", description = "User profile not found and could not be synced")
  @PreAuthorize("isAuthenticated()") // Ensures a user is logged in
  public ResponseEntity<UserDto> getCurrentUserProfile(Authentication authentication) {
    String clerkId =
        authentication.getName(); // Spring Security puts the token's subject (clerkId) here
    ...
    return userService
        ...
        .orElseGet(
            () -> {
              // User not found - this shouldn't happen if JwtUserSyncFilter ran
              // But as a fallback, return 404 with helpful message
              log.warn("User with clerkId {} not found in database after authentication", clerkId);
              return ResponseEntity.notFound().build();
            });
  }

  /**
   * Retrieves a paginated list of all users. This is an administrative endpoint and should be
   * protected accordingly.
   */
  /**
   * Update user preferences for the authenticated user. Preferences include theme, daily goals,
   * notifications, etc.
   */
  ...
  public ResponseEntity<UserDto> updatePreferences(
      Authentication authentication, @Valid @RequestBody UserPreferencesDto preferencesDto) {
  ...
  /**
   * Delete the authenticated user's account. Requires email confirmation to prevent accidental
   * deletion. This permanently deletes all user data including favorites, playback history, and the
   * Clerk account.
   */
  @DeleteMapping("/me")
  @Operation(
      ...
      description =
          "Permanently deletes the authenticated user's account and all associated data. Requires email confirmation.")
  @SecurityRequirement(name = "bearerAuth")
  ...
  public ResponseEntity<Void> deleteAccount(
      Authentication authentication, @RequestParam("confirmEmail") String confirmEmail) {
    String clerkId = authentication.getName();
